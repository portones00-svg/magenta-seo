...
const PAGINAS_PRUEBA = ['a-domicilio-en-puente-alto.html', 'en-la-serena.html', 'a-domicilio-en-vitacura.html'];
...
(async () => {
  for (const pagina of PAGINAS_PRUEBA) {
    ...
    if (!h1Match) { console.log('⚠️ No se encontro H1 en', pagina); continue; }

    const h1 = h1Match[1];
    const idxEn = h1.lastIndexOf(' en ');
    if (idxEn === -1) { console.log('⚠️ No se pudo extraer ciudad del H1:', h1); continue; }
    ...
    console.log('✅', pagina, '→ Schema.org agregado para ciudad:', ciudad);
  }
})();
